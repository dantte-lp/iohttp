...
|------|-------------|-------------|------------|
| HEADER | `IO_TIMEOUT_HEADER` | 30 с | Время на получение полных HTTP-заголовков |
| BODY | `IO_TIMEOUT_BODY` | 60 с | Время на получение полного тела запроса |
| KEEPALIVE | `IO_TIMEOUT_KEEPALIVE` | 65 с | Время простоя между конвейерными запросами |

Фаза таймаута переключается автоматически по мере обработки запроса:

1. Новое соединение начинается в фазе `IO_TIMEOUT_HEADER`
2. Когда заголовки разобраны, но тело ещё не получено, происходит переход в `IO_TIMEOUT_BODY`
3. После отправки ответа на keep-alive соединении происходит переход в `IO_TIMEOUT_KEEPALIVE`
4. При поступлении следующего запроса фаза сбрасывается на `IO_TIMEOUT_HEADER`
...
> [!IMPORTANT]
> Поле `timeout_ts` находится в `io_conn_t` (не на стеке), потому что ядро читает его
...
|----------|------|-------------|---------|
| Таймаут заголовков | `io_server_config_t.header_timeout_ms` | 30000 | мс |
| Таймаут тела | `io_server_config_t.body_timeout_ms` | 60000 | мс |
| Таймаут keep-alive | `io_server_config_t.keepalive_timeout_ms` | 65000 | мс |
...
| Параметр | Поле | По умолчанию | Единица |
|----------|------|-------------|---------|
| Макс. размер заголовков | `io_server_config_t.max_header_size` | 8192 | байт |
| Макс. размер тела | `io_server_config_t.max_body_size` | 1048576 | байт |
...
|--------|----------|-----------------|
| `SIGTERM` | Плавное завершение (drain) | `IO_SHUTDOWN_DRAIN` |
| `SIGQUIT` | Немедленное закрытие | `IO_SHUTDOWN_IMMEDIATE` |
...
2. Перевести все соединения `IO_CONN_HTTP_ACTIVE` в состояние `IO_CONN_DRAINING`
...
> [!CAUTION]
> `signalfd` создаётся внутри `io_server_run()`. Если вы используете
> `io_server_run_once()` в собственном событийном цикле, вы должны обрабатывать
...
Модуль `io_log` предоставляет логирование с фильтрацией по уровням, тегами модулей
...
|---------|-------------|-------------------|------------|
| ERROR | `IO_LOG_ERROR` | 0 | Невосстановимые сбои, исчерпание ресурсов |
| WARN | `IO_LOG_WARN` | 1 | Отклонённые запросы, переполнение пула, некорректный ввод |
| INFO | `IO_LOG_INFO` | 2 | Запуск/остановка сервера, адрес прослушивания, завершение |
| DEBUG | `IO_LOG_DEBUG` | 3 | События по соединениям, декодирование PROXY, таймауты |
...
Идентификатор выделяется из арены текущего запроса (`io_ctx_sprintf`), поэтому
...
Поле `src_addr` сохраняется в `io_conn_t.proxy_addr`, а `proxy_used` устанавливается
...
|----------|------|-------------|
| Включить PROXY protocol | `io_server_config_t.proxy_protocol` | `false` |
...
|---------|-----------|---------------------|
| Связанные таймауты | `test_io_server.c` | Переходы фаз таймаутов, обработка CQE для `-ECANCELED` и `-ETIME` |
| Ограничения запросов | `test_io_server.c` | 431 при превышении заголовков, 413 при превышении тела |
| Обработка сигналов | `test_io_server.c` | Drain по SIGTERM, немедленное завершение по SIGQUIT, жизненный цикл signalfd |
| Структурированное логирование | `test_io_log.c` | Фильтрация по уровням, пользовательский обработчик, строки названий уровней |
| Идентификатор запроса | `test_io_server.c` | Формат генерации, передача входящего ID, выделение из арены |
| PROXY protocol | `test_io_proxy_proto.c` | Декодирование v1, декодирование v2, неполные данные, некорректный ввод |
...
по умолчанию, устанавливаемые `io_server_config_init()`. Существующий код, вызывающий
`io_server_config_init()` перед настройкой полей, автоматически получит новые значения
по умолчанию.
...
| Изменение | Влияние |
|-----------|---------|
| `io_server_config_t` получает `header_timeout_ms`, `body_timeout_ms`, `keepalive_timeout_ms` | Значения по умолчанию задаются `io_server_config_init()` |
| `io_server_config_t` получает `max_header_size`, `max_body_size` | По умолчанию: 8 КиБ / 1 МиБ |
| `io_server_config_t` получает `proxy_protocol` | По умолчанию: `false` (отключено) |
| `io_conn_t` получает `timeout_phase`, `timeout_ts`, `proxy_addr`, `proxy_used` | Внутренние поля, не являются частью публичного API |
| Добавлен API `io_log_*` | Опционально -- сервер использует его внутренне, приложения могут адаптировать |
...
> сериализации ответа (побеждает последний вызов `io_response_set_header`). Чтобы
