# iohttp — Agent Contract

This is the canonical agent contract for the iohttp repository. Claude Code,
Codex and Gemini CLI all read it. Follow it exactly for every change.
...
Develop **inside the container**. Build the image, then run it with
`seccomp=unconfined` (required — `io_uring_setup` needs it):
...
Primary workflow (CMake is the build system):
...
```bash
cmake --build --preset clang-debug --target format        # clang-format (apply)
cmake --build --preset clang-debug --target format-check  # clang-format (verify)
...
Run `./scripts/quality.sh` before submitting changes. **ALL development and
compilation MUST happen inside the podman container** — never run build commands
...
`src/` is split by domain. Public headers live in `include/iohttp/`; internal
declarations live under `src/`.
...
- **Translation units**: prefer small, single-purpose translation units
- **Tests**: Unity framework, `test_module_action_expected()`, typed assertions, cleanup resources
- Follow `.clang-format`; C extensions are disabled.
...
wolfSSL is dual-licensed: GPLv2+ (open source) or commercial. GPLv2-or-later is
forward-compatible with GPLv3, so iohttp's GPLv3 license is compatible with
wolfSSL's open-source license. No commercial license needed for GPLv3 projects.

## Testing Rules

- All new code MUST have unit tests (Unity framework, enabled via `IOHTTP_BUILD_TESTS`)
- Test files: `tests/unit/test_<module>.c`, named `test_module_action_expected()`
- Sanitizer presets: `clang-asan`, `clang-msan`, `clang-tsan`
- Sanitizer policy: ASan+UBSan (every commit), MSan (Clang, every commit), TSan (before merge)
...
After completing each sprint, run the **full quality pipeline** inside the
container before considering the sprint done. Single command:
...
public API, verification evidence, release automation, or published documentation.
...
See `docs/en/04-framework-comparison.md` for detailed comparison with 12
frameworks (Gin, Fiber, Echo, Hertz, FastAPI, Actix-web, Axum, Kestrel, Spring
Boot, Quarkus).

**Key differentiators:** io_uring (exclusive), HTTP/1.1+2+3 in C (only Kestrel
matches), wolfSSL+QUIC (exclusive), no GC/runtime, ~15K own LOC.

## Git Workflow

- Branch naming: `feature/description`, `fix/issue-description`
- Commit style: conventional commits (`feat:`, `fix:`, `refactor:`, `test:`, `docs:`) with short imperative subjects
- All commits must pass: clang-format, clang-tidy, unit tests
- PRs must describe the behavior change, commands run, and analyzer results
