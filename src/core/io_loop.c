...
int io_loop_config_validate(const io_loop_config_t *cfg)
{
    if (cfg == nullptr) {
        return -EINVAL;
    }
...
io_loop_t *io_loop_create(const io_loop_config_t *cfg)
{
    if (cfg == nullptr) {
        return nullptr;
    }
    if (io_loop_config_validate(cfg) != 0) {
        return nullptr;
    }

    io_loop_t *loop = calloc(1, sizeof(*loop));
    if (loop == nullptr) {
        return nullptr;
    }
    loop->config = *cfg;
...
int io_loop_submit_nop(io_loop_t *loop)
{
    if (loop == nullptr) {
        return -EINVAL;
    }
    ...
    io_uring_sqe_set_data64(sqe, IO_ENCODE_USERDATA(0, IO_OP_NOP));
...
int io_loop_run_once(io_loop_t *loop, uint32_t timeout_ms)
{
    if (loop == nullptr) {
        return -EINVAL;
    }
    ...
    io_uring_for_each_cqe(&loop->ring, head, cqe) {
        ...
        if (op == IO_OP_TIMEOUT) {
            uint64_t timer_id = IO_DECODE_ID(ud);
            if (timer_id < IO_MAX_TIMERS && loop->timers[timer_id].active) {
                      ...
                      void (*cb)(void *), void *data)
{
    ...
    int slot = -1;
    for (uint32_t i = 0; i < IO_MAX_TIMERS; i++) {
    ...
    io_uring_sqe_set_data64(sqe, IO_ENCODE_USERDATA((uint64_t)slot, IO_OP_TIMEOUT));

    return slot;
}

int io_loop_cancel_timer(io_loop_t *loop, int timer_id)
{
    if (loop == nullptr) {
        return -EINVAL;
    }
    if (timer_id < 0 || (uint32_t)timer_id >= IO_MAX_TIMERS) {
        return -EINVAL;
    }
...
int io_loop_register_buffers(io_loop_t *loop, struct iovec *iovs, unsigned nr)
{
...
int io_loop_unregister_buffers(io_loop_t *loop)
{
    if (loop == nullptr) {
        return -EINVAL;
    }
...
int io_loop_register_files(io_loop_t *loop, int *fds, unsigned nr)
{
...
int io_loop_unregister_files(io_loop_t *loop)
{
    if (loop == nullptr) {
        return -EINVAL;
    }
