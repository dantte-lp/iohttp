...
#define IO_OP_BITS 8
#define IO_ENCODE_USERDATA(id, op) \
    (((uint64_t)(id) << IO_OP_BITS) | (uint8_t)(op))
 ...
 */
[[nodiscard]] int io_loop_add_timer(io_loop_t *loop, uint32_t ms,
 ...
 * @return 0 on success, negative errno on error.
 */
[[nodiscard]] int io_loop_setup_buf_ring(io_loop_t *loop, uint16_t bgid,
 ...
 * @return 0 on success, negative errno on error.
 */
[[nodiscard]] int io_loop_register_buffers(io_loop_t *loop,
                                           struct iovec *iovs, unsigned nr);
