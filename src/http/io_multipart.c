 ...
 */
static bool ci_prefix(const char *str, size_t str_len, const char *prefix,
                       size_t prefix_len)
{
 ...
 */
static bool extract_param(const char *str, size_t str_len,
 ...
 */
static int parse_part_headers(const char *headers, size_t headers_len,
                               io_multipart_part_t *part)
{
    ...
    while (p < end) {
        ...
        if (ci_prefix(p, line_len, cd, cd_len)) {
            ...
            extract_param(val, val_len, "name=", 5,
                          &part->name, &part->name_len);
            extract_param(val, val_len, "filename=", 9,
                          &part->filename, &part->filename_len);
        }
...
int io_multipart_boundary(const char *content_type,
                           const char **boundary,
                           size_t *boundary_len)
{
    ...
    if (!extract_param(content_type, ct_len, "boundary=", 9,
...
int io_multipart_parse(const uint8_t *body, size_t body_len,
                        ...
                        uint32_t *part_count)
{
    ...
    *part_count = 0;
    ...
    while (remain > 0) {
        ...
        const uint8_t *hdr_end = find_bytes(pos, remain,
                                             (const uint8_t *)"\r\n\r\n", 4);
        if (!hdr_end)
            return -EINVAL;
        ...
        const uint8_t *next_delim = find_bytes(data_start, data_remain,
                                                delim, delim_len);
        if (!next_delim)
            return -EINVAL;
