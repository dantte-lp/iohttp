 ...
 * @brief Unit tests for typed parameter extraction from io_request_t.
 */
...
void test_param_string(void)
{
    ...
    const char *val = io_request_param(&req, "id");
    TEST_ASSERT_NOT_NULL(val);
    TEST_ASSERT_EQUAL_STRING("alice", val);

    /* nonexistent key */
    TEST_ASSERT_NULL(io_request_param(&req, "missing"));

    /* nullptr safety */
    TEST_ASSERT_NULL(io_request_param(nullptr, "id"));
    TEST_ASSERT_NULL(io_request_param(&req, nullptr));
}
...
void test_param_i64_valid(void)
{
    ...
    int64_t out = 0;
    int rc = io_request_param_i64(&req, "id", &out);
...
void test_param_i64_negative(void)
{
    ...
    int64_t out = 0;
    int rc = io_request_param_i64(&req, "offset", &out);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_param_i64_overflow(void)
{
    ...
    int64_t out = 0;
    int rc = io_request_param_i64(&req, "big", &out);
...
void test_param_u64_valid(void)
{
    ...
    uint64_t out = 0;
    int rc = io_request_param_u64(&req, "size", &out);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_param_bool_true(void)
{
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        out = false;
        int rc = io_request_param_bool(&req, "flag", &out);
        TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_param_missing(void)
{
    ...
    TEST_ASSERT_NULL(io_request_param(&req, "id"));

    int64_t i64 = 0;
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_request_param_i64(&req, "id", &i64));

    uint64_t u64 = 0;
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_request_param_u64(&req, "id", &u64));

    bool b = false;
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_request_param_bool(&req, "id", &b));
}
...
void test_param_count(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT32(0, io_request_param_count(&req));
    TEST_ASSERT_EQUAL_UINT32(0, io_request_param_count(nullptr));

    setup_param(&req, 0, "a", "1");
    TEST_ASSERT_EQUAL_UINT32(1, io_request_param_count(&req));

    setup_param(&req, 1, "b", "2");
    TEST_ASSERT_EQUAL_UINT32(2, io_request_param_count(&req));

    setup_param(&req, 2, "c", "3");
    TEST_ASSERT_EQUAL_UINT32(3, io_request_param_count(&req));
}
