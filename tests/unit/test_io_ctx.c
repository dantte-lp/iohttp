/**
 * @file test_io_ctx.c
 * @brief Unit tests for io_ctx unified request context.
 */
...
void test_ctx_init_destroy(void)
{
    ...
    TEST_ASSERT_EQUAL_size_t(IO_CTX_ARENA_DEFAULT, ctx.arena.size);
    ...
    /* nullptr inputs */
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_ctx_init(nullptr, &req, &resp, nullptr));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_ctx_init(&ctx, nullptr, &resp, nullptr));
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_ctx_init(&ctx, &req, nullptr, nullptr));

    io_ctx_destroy(&ctx);

    /* destroying nullptr is safe */
    io_ctx_destroy(nullptr);
}
...
void test_ctx_set_get_value(void)
{
    ...
    int data = 42;
    TEST_ASSERT_EQUAL_INT(0, io_ctx_set(&ctx, "user", &data));

    void *got = io_ctx_get(&ctx, "user");
...
void test_ctx_set_max_values(void)
{
    ...
    for (uint32_t i = 0; i < IO_CTX_MAX_VALUES; i++) {
        vals[i] = (int)i;
        TEST_ASSERT_EQUAL_INT(0, io_ctx_set(&ctx, keys[i], &vals[i]));
    }

    /* 17th should fail */
    int extra = 99;
    TEST_ASSERT_EQUAL_INT(-ENOSPC, io_ctx_set(&ctx, "overflow", &extra));

    /* verify all 16 are still accessible */
    for (uint32_t i = 0; i < IO_CTX_MAX_VALUES; i++) {
        TEST_ASSERT_EQUAL_PTR(&vals[i], io_ctx_get(&ctx, keys[i]));
    }
...
void test_ctx_get_missing_returns_null(void)
{
    ...
    TEST_ASSERT_NULL(io_ctx_get(&ctx, "nonexistent"));
    TEST_ASSERT_NULL(io_ctx_get(nullptr, "key"));
    TEST_ASSERT_NULL(io_ctx_get(&ctx, nullptr));
...
void test_ctx_set_replaces_existing(void)
{
    ...
    int v1 = 1, v2 = 2;
    TEST_ASSERT_EQUAL_INT(0, io_ctx_set(&ctx, "key", &v1));
    TEST_ASSERT_EQUAL_PTR(&v1, io_ctx_get(&ctx, "key"));

    TEST_ASSERT_EQUAL_INT(0, io_ctx_set(&ctx, "key", &v2));
    TEST_ASSERT_EQUAL_PTR(&v2, io_ctx_get(&ctx, "key"));
...
void test_ctx_param_delegates(void)
{
    ...
    const char *val = io_ctx_param(&ctx, "id");
    TEST_ASSERT_NOT_NULL(val);
    TEST_ASSERT_EQUAL_STRING("42", val);

    /* typed extraction */
    int64_t i64val;
    TEST_ASSERT_EQUAL_INT(0, io_ctx_param_i64(&ctx, "id", &i64val));
    TEST_ASSERT_EQUAL_INT64(42, i64val);

    uint64_t u64val;
    TEST_ASSERT_EQUAL_INT(0, io_ctx_param_u64(&ctx, "id", &u64val));
...
void test_ctx_query_delegates(void)
{
    ...
    const char *val = io_ctx_query(&ctx, "page");
    TEST_ASSERT_NOT_NULL(val);
...
void test_ctx_header_delegates(void)
{
    ...
    const char *val = io_ctx_header(&ctx, "Content-Type");
    TEST_ASSERT_NOT_NULL(val);
...
void test_ctx_no_content_sets_204(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, io_ctx_no_content(&ctx));
...
void test_ctx_arena_alloc(void)
{
    ...
    void *p1 = io_ctx_alloc(&ctx, 64);
    TEST_ASSERT_NOT_NULL(p1);

    void *p2 = io_ctx_alloc(&ctx, 128);
    ...
    TEST_ASSERT_NULL(io_ctx_alloc(&ctx, 0));
...
void test_ctx_arena_overflow_realloc(void)
{
    ...
    void *p = io_ctx_alloc(&ctx, IO_CTX_ARENA_DEFAULT + 1);
    TEST_ASSERT_NOT_NULL(p);
    TEST_ASSERT_GREATER_THAN(IO_CTX_ARENA_DEFAULT, ctx.arena.size);

    /* can still allocate more */
    void *p2 = io_ctx_alloc(&ctx, 64);
    TEST_ASSERT_NOT_NULL(p2);
...
void test_ctx_sprintf_arena(void)
{
    ...
    char *s = io_ctx_sprintf(&ctx, "hello %s, id=%d", "world", 42);
