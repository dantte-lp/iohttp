/**
 * @file test_io_log.c
 * @brief Unit tests for io_log structured logging module.
 */
...
void setUp(void)
{
    ...
    io_log_set_sink(test_sink, &g_state);
}
...
{
    io_log(IO_LOG_INFO, "server", "hello %s", "world");

    TEST_ASSERT_EQUAL_INT(1, g_state.call_count);
    TEST_ASSERT_EQUAL_UINT8(IO_LOG_INFO, g_state.last_level);
...
void test_log_level_filtering(void)
{
    ...
    io_log(IO_LOG_DEBUG, "test", "should be filtered");
    TEST_ASSERT_EQUAL_INT(0, g_state.call_count);

    io_log(IO_LOG_INFO, "test", "also filtered");
    TEST_ASSERT_EQUAL_INT(0, g_state.call_count);

    io_log(IO_LOG_WARN, "test", "should pass");
    TEST_ASSERT_EQUAL_INT(1, g_state.call_count);
    TEST_ASSERT_EQUAL_STRING("should pass", g_state.last_message);

    io_log(IO_LOG_ERROR, "test", "also passes");
...
void test_log_level_names(void)
{
    ...
    TEST_ASSERT_EQUAL_STRING("UNKNOWN", io_log_level_name((io_log_level_t)99));
}
...
{
    IO_LOG_ERROR("core", "error %d", 42);
    TEST_ASSERT_EQUAL_INT(1, g_state.call_count);
    TEST_ASSERT_EQUAL_UINT8(IO_LOG_ERROR, g_state.last_level);
    TEST_ASSERT_EQUAL_STRING("core", g_state.last_module);
    TEST_ASSERT_EQUAL_STRING("error 42", g_state.last_message);

    IO_LOG_WARN("net", "warning");
    TEST_ASSERT_EQUAL_INT(2, g_state.call_count);
    TEST_ASSERT_EQUAL_UINT8(IO_LOG_WARN, g_state.last_level);

    IO_LOG_INFO("http", "info");
    TEST_ASSERT_EQUAL_INT(3, g_state.call_count);
    TEST_ASSERT_EQUAL_UINT8(IO_LOG_INFO, g_state.last_level);

    IO_LOG_DEBUG("tls", "debug");
    TEST_ASSERT_EQUAL_INT(4, g_state.call_count);
    TEST_ASSERT_EQUAL_UINT8(IO_LOG_DEBUG, g_state.last_level);
}
...
void test_log_get_set_level(void)
{
    ...
    io_log_set_level(IO_LOG_DEBUG);
    TEST_ASSERT_EQUAL_UINT8(IO_LOG_DEBUG, io_log_get_level());

    io_log_set_level(IO_LOG_WARN);
    TEST_ASSERT_EQUAL_UINT8(IO_LOG_WARN, io_log_get_level());

    io_log_set_level(IO_LOG_INFO);
    TEST_ASSERT_EQUAL_UINT8(IO_LOG_INFO, io_log_get_level());
}
...
void test_log_null_inputs(void)
{
    ...
    io_log(IO_LOG_INFO, nullptr, "msg with null module");
    TEST_ASSERT_EQUAL_INT(1, g_state.call_count);
