/**
 * @file test_io_loop.c
 * @brief Unit tests for io_loop ring management wrapper.
 */
...
void test_loop_config_validate_valid(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(0, io_loop_config_validate(&cfg));
}
...
void test_loop_config_validate_invalid(void)
{
    ...
    cfg.queue_depth = 0;
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_loop_config_validate(&cfg));
    ...
    /* nullptr config */
    TEST_ASSERT_EQUAL_INT(-EINVAL, io_loop_config_validate(nullptr));
}
...
void test_loop_create_destroy(void)
{
    ...
    io_loop_t *loop = io_loop_create(&cfg);
    TEST_ASSERT_NOT_NULL(loop);
    TEST_ASSERT_NOT_NULL(io_loop_ring(loop));

    io_loop_destroy(loop);

    /* Destroying nullptr should not crash */
    io_loop_destroy(nullptr);
}
...
void test_loop_nop_submit(void)
{
    ...
    int ret = io_loop_submit_nop(loop);
    TEST_ASSERT_EQUAL_INT(0, ret);

    /* Run once to drain the CQE */
    ret = io_loop_run_once(loop, 1000);
...
void test_loop_timer_fires(void)
{
    ...
    int timer_id = io_loop_add_timer(loop, 10, timer_callback, &timer_fired_flag);
    TEST_ASSERT_GREATER_OR_EQUAL(0, timer_id);

    /* Run loop — timer should fire within 10ms + some slack */
    int ret = io_loop_run_once(loop, 1000);
    TEST_ASSERT_GREATER_THAN(0, ret);
...
void test_loop_timer_cancel(void)
{
    ...
    timer_fired_flag = false;
    int timer_id = io_loop_add_timer(loop, 5000, timer_callback, &timer_fired_flag);
    TEST_ASSERT_GREATER_OR_EQUAL(0, timer_id);

    int ret = io_loop_cancel_timer(loop, timer_id);
    TEST_ASSERT_EQUAL_INT(0, ret);

    /* Run once to process the cancel + timeout CQEs */
    ret = io_loop_run_once(loop, 500);
    TEST_ASSERT_GREATER_THAN(0, ret);
...
void test_loop_linked_timeout_fires(void)
{
    ...
    struct io_uring *ring = io_loop_ring(loop);
    ...
    io_uring_sqe_set_data64(sqe, IO_ENCODE_USERDATA(1, IO_OP_RECV));
    ...
    for (int i = 0; i < 3 && total < 2; i++) {
        int ret = io_loop_run_once(loop, 2000);
        TEST_ASSERT_GREATER_THAN(0, ret);
        total += ret;
    }
...
void test_loop_linked_timeout_no_fire(void)
{
    ...
    io_uring_sqe_set_data64(sqe, IO_ENCODE_USERDATA(2, IO_OP_NOP));
    sqe->flags |= IOSQE_IO_LINK;
...
void test_loop_provided_buffer_ring(void)
{
    ...
    int ret = io_loop_setup_buf_ring(loop, 0, 16, 4096);
    TEST_ASSERT_EQUAL_INT(0, ret);

    /* Verify we can get buffer pointers */
    uint8_t *b0 = io_loop_get_buf(loop, 0, 0);
    TEST_ASSERT_NOT_NULL(b0);

    uint8_t *b1 = io_loop_get_buf(loop, 0, 1);
    ...
    TEST_ASSERT_NULL(io_loop_get_buf(loop, 0, 16));

    /* Wrong bgid returns nullptr */
    TEST_ASSERT_NULL(io_loop_get_buf(loop, 1, 0));

    /* Return a buffer and verify no crash */
    io_loop_return_buf(loop, 0, 0, b0, 4096);
...
void test_loop_register_buffers(void)
{
    ...
    ret = io_loop_unregister_buffers(loop);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
void test_loop_register_files(void)
{
    ...
    int ret = io_loop_register_files(loop, pipefd, 2);
    TEST_ASSERT_EQUAL_INT(0, ret);

    ret = io_loop_unregister_files(loop);
    TEST_ASSERT_EQUAL_INT(0, ret);
...
void test_loop_userdata_encoding(void)
{
    ...
    uint64_t ud = IO_ENCODE_USERDATA(42, IO_OP_RECV);
    TEST_ASSERT_EQUAL_UINT8(IO_OP_RECV, IO_DECODE_OP(ud));
    TEST_ASSERT_EQUAL_UINT64(42, IO_DECODE_ID(ud));

    /* Zero values */
    ud = IO_ENCODE_USERDATA(0, IO_OP_NOP);
    TEST_ASSERT_EQUAL_UINT8(IO_OP_NOP, IO_DECODE_OP(ud));
    TEST_ASSERT_EQUAL_UINT64(0, IO_DECODE_ID(ud));

    /* Large ID */
    ud = IO_ENCODE_USERDATA(0xFFFFFFFFULL, IO_OP_SEND_ZC);
    TEST_ASSERT_EQUAL_UINT8(IO_OP_SEND_ZC, IO_DECODE_OP(ud));
    TEST_ASSERT_EQUAL_UINT64(0xFFFFFFFFULL, IO_DECODE_ID(ud));

    /* All op types round-trip correctly */
    for (uint8_t op = 0; op <= IO_OP_CANCEL; op++) {
        ud = IO_ENCODE_USERDATA(100, op);
        TEST_ASSERT_EQUAL_UINT8(op, IO_DECODE_OP(ud));
        TEST_ASSERT_EQUAL_UINT64(100, IO_DECODE_ID(ud));
    }
}
