...
void test_multipart_single_field(void)
{
    ...
    int rc = io_multipart_parse((const uint8_t *)body, strlen(body),
    ...
    TEST_ASSERT_EQUAL_STRING_LEN("hello world", (const char *)parts[0].data,
                                 11);
}
...
void test_multipart_file_upload(void)
{
    ...
    TEST_ASSERT_EQUAL_STRING_LEN("test.txt", parts[0].filename,
                                 parts[0].filename_len);
    TEST_ASSERT_NOT_NULL(parts[0].content_type);
    TEST_ASSERT_EQUAL_STRING_LEN("text/plain", parts[0].content_type,
                                 parts[0].content_type_len);
    TEST_ASSERT_EQUAL_size_t(18, parts[0].data_len);
    TEST_ASSERT_EQUAL_STRING_LEN("file contents here",
                                 (const char *)parts[0].data, 18);
}
...
void test_multipart_multiple_parts(void)
{
    ...
    TEST_ASSERT_EQUAL_STRING_LEN("Alice", (const char *)parts[0].data,
                                 parts[0].data_len);

    TEST_ASSERT_EQUAL_STRING_LEN("age", parts[1].name, parts[1].name_len);
    TEST_ASSERT_EQUAL_STRING_LEN("30", (const char *)parts[1].data,
                                 parts[1].data_len);

    TEST_ASSERT_EQUAL_STRING_LEN("city", parts[2].name, parts[2].name_len);
    TEST_ASSERT_EQUAL_STRING_LEN("Berlin", (const char *)parts[2].data,
                                 parts[2].data_len);

    TEST_ASSERT_EQUAL_STRING_LEN("avatar", parts[3].name, parts[3].name_len);
    TEST_ASSERT_NOT_NULL(parts[3].filename);
    TEST_ASSERT_EQUAL_STRING_LEN("pic.png", parts[3].filename,
                                 parts[3].filename_len);
    TEST_ASSERT_EQUAL_STRING_LEN("PNG_DATA", (const char *)parts[3].data,
                                 parts[3].data_len);
}
...
void test_multipart_empty_body(void)
{
    ...
    int rc = io_multipart_parse((const uint8_t *)"", 0,
                                BOUNDARY, strlen(BOUNDARY),
                                &cfg, parts, &count);
    TEST_ASSERT_EQUAL_INT(0, rc);
...
void test_multipart_missing_boundary(void)
{
    ...
    int rc = io_multipart_boundary("multipart/form-data",
                                   &bptr, &boundary_len);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);

    /* not multipart at all */
    rc = io_multipart_boundary("application/json",
                               &bptr, &boundary_len);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);
...
void test_multipart_oversized_part(void)
{
    ...
    int rc = io_multipart_parse(buf, off, BOUNDARY, strlen(BOUNDARY),
                                &cfg, parts, &count);
...
void test_multipart_malformed(void)
{
    ...
    int rc = io_multipart_parse((const uint8_t *)garbage, strlen(garbage),
                                BOUNDARY, strlen(BOUNDARY),
                                &cfg, parts, &count);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);
    ...
    count = 8;
    const char *no_disposition =
        "------TestBoundary\r\n"
        "Content-Type: text/plain\r\n"
        "\r\n"
        "data\r\n"
        "------TestBoundary--\r\n";
    rc = io_multipart_parse((const uint8_t *)no_disposition,
                            strlen(no_disposition),
                            BOUNDARY, strlen(BOUNDARY),
                            &cfg, parts, &count);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);
    ...
    rc = io_multipart_parse((const uint8_t *)truncated,
                            strlen(truncated),
                            BOUNDARY, strlen(BOUNDARY),
                            &cfg, parts, &count);
    TEST_ASSERT_EQUAL_INT(-EINVAL, rc);
