...
void test_tls_buffer_data_roundtrip(void)
{
	...
	int wret = io_tls_write(sconn, (const uint8_t *)server_msg,
				strlen(server_msg));
	...
	int cwret = wolfSSL_write(client->ssl, client_msg,
...
void test_tls_buffer_multiple_messages(void)
{
	...
	const char *messages[5] = {
		...
		"Second message here",
		"Third",
		"A slightly longer fourth message payload",
		"Five",
	};
...
void test_tls_buffer_large_payload(void)
{
	...
	while (total_written < PAYLOAD_SIZE || total_read < PAYLOAD_SIZE) {
		...
		if (total_read < PAYLOAD_SIZE) {
			...
			int rret = wolfSSL_read(client->ssl,
						received + total_read,
						(int)remaining);
			if (rret > 0) {
...
void test_tls_buffer_shutdown(void)
{
	...
	TEST_ASSERT_TRUE_MESSAGE(shutdown_done,
				 "TLS shutdown did not complete");
