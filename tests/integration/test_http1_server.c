...
static void init_cert_paths(void)
{
	...
	snprintf(server_cert, sizeof(server_cert), "%s/server-cert.pem",
		 TEST_CERTS_DIR);
	snprintf(server_key, sizeof(server_key), "%s/server-key.pem",
		 TEST_CERTS_DIR);
...
void test_http1_full_request_response(void)
{
	...
	io_request_t req;
	int consumed = io_http1_parse_request((const uint8_t *)raw,
					      strlen(raw), &req);
	...
	TEST_ASSERT_TRUE(buf_contains(out, (size_t)slen,
...
void test_http1_keepalive_multiple(void)
{
	...
	const char *expected_paths[] = { "/path/one", "/path/two",
					 "/path/three" };
	size_t expected_lens[] = { 9, 9, 11 };

	for (int i = 0; i < 3; i++) {
		io_request_t req;
		int consumed = io_http1_parse_request(
			(const uint8_t *)requests[i], strlen(requests[i]),
			&req);
		TEST_ASSERT_GREATER_THAN(0, consumed);
		TEST_ASSERT_EQUAL(IO_METHOD_GET, req.method);
		TEST_ASSERT_EQUAL_size_t(expected_lens[i], req.path_len);
		TEST_ASSERT_EQUAL_STRING_LEN(expected_paths[i], req.path,
					     req.path_len);
		TEST_ASSERT_TRUE_MESSAGE(req.keep_alive,
					 "HTTP/1.1 should default keep-alive");
	}
}
...
void test_http1_connection_close(void)
{
	...
	TEST_ASSERT_GREATER_THAN(0, consumed);
	TEST_ASSERT_EQUAL(IO_METHOD_GET, req.method);
	TEST_ASSERT_FALSE_MESSAGE(req.keep_alive,
				  "Connection: close must set keep_alive=false");
...
void test_http1_post_with_body(void)
{
	...
	io_request_t req;
	int consumed =
		io_http1_parse_request((const uint8_t *)raw, (size_t)rawlen, &req);
	TEST_ASSERT_GREATER_THAN(0, consumed);
	...
	TEST_ASSERT_EQUAL_STRING_LEN(body, (const char *)req_body,
				     strlen(body));
...
static test_tls_client_t *client_create(void)
{
	...
	if (wolfSSL_CTX_load_verify_locations(client->ctx, TEST_CA_CERT,
					      nullptr) != WOLFSSL_SUCCESS) {
...
static void shuttle_data(io_tls_conn_t *server, test_tls_client_t *client)
{
	...
	if (slen > 0) {
		size_t space =
			IO_TLS_CIPHER_BUF_SIZE - client->cipher_in_len;
		size_t copy = slen < space ? slen : space;
		if (copy > 0) {
			memcpy(client->cipher_in_buf + client->cipher_in_len,
			       sout, copy);
	...
	if (client->cipher_out_len > 0) {
		(void)io_tls_feed_input(server, client->cipher_out_buf,
					client->cipher_out_len);
		client->cipher_out_len = 0;
	}
}

static bool do_buffer_handshake(io_tls_conn_t *server,
				test_tls_client_t *client)
{
	...
	for (int i = 0; i < 200; i++) {
		...
		if (!client_done) {
			...
			if (ret == WOLFSSL_SUCCESS) {
				...
				int err =
					wolfSSL_get_error(client->ssl, ret);
...
void test_http1_tls_request_response(void)
{
	...
	io_request_t req;
	int consumed =
		io_http1_parse_request(plaintext, (size_t)rret, &req);
	TEST_ASSERT_GREATER_THAN(0, consumed);
	TEST_ASSERT_EQUAL(IO_METHOD_GET, req.method);
	...
	int slen =
		io_http1_serialize_response(&resp, resp_buf, sizeof(resp_buf));
	TEST_ASSERT_GREATER_THAN(0, slen);
	...
	TEST_ASSERT_TRUE(buf_contains((const uint8_t *)client_buf,
				      (size_t)rret, "HTTP/1.1 200 OK"));
	TEST_ASSERT_TRUE(buf_contains((const uint8_t *)client_buf,
				      (size_t)rret, "{\"running\":true}"));
...
void test_http1_proxy_then_request(void)
{
	...
	io_request_t req;
	int http_consumed = io_http1_parse_request(http_start, http_remaining,
						   &req);
